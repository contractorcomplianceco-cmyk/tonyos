    ...
    <div className="space-y-8 pb-12 max-w-5xl mx-auto">
      ...
      <div className="border-b border-border pb-6">
        ...
        <div className="flex items-center gap-4 text-sm font-semibold">
          ...
          <span className={`px-2.5 py-1 rounded-[2px] border uppercase tracking-widest text-[10px] font-mono font-bold ${review.status === 'published' ? 'bg-emerald-500/10 text-emerald-700 border-emerald-600/30' : 'bg-amber-500/10 text-amber-700 border-amber-600/30'}`}>
            {review.status}
          </span>
        </div>
      </div>
        ...
        {review.watchItems && review.watchItems.length > 0 && (
          <div className="bg-amber-500/10 border border-amber-500/30 rounded shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-amber-500/30 flex items-center gap-3">
              ...
              <h3 className="text-base font-bold text-amber-800 tracking-tight">Watch Items</h3>
            </div>
            <div className="p-6">
              <ul className="space-y-4">
                {review.watchItems.map((w, i) => (
                  <li key={i} className="text-sm text-amber-900/90 pl-4 border-l-[3px] border-amber-500 font-medium leading-relaxed">{w}</li>
                ))}
              </ul>
            </div>
          </div>
